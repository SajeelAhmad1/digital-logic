...
type MobileTab = 'learn' | 'code' | 'output';
...
export default function LessonPage() {
  ...
  const handleRun = async () => {
    ...
    setRunning(true);
    setOutput('Running...');
    // On mobile, auto-switch to output tab when running
    setMobileTab('output');
    try {
    ...
    <TooltipProvider>
      <div className="flex flex-col h-screen bg-[#1e1e2e]">
        ...
        <style>{`
          ...
          /* Desktop layout */
          .lesson-desktop-body { display: flex; }
          .lesson-left-panel { display: flex; }
          .lesson-editor-panel { display: flex; }
          .lesson-output-panel { display: flex; }
          .lesson-mobile-tabs { display: none; }
          .lesson-mobile-tab-content { display: none; }

          @media (max-width: 768px) {
            /* Hide desktop multi-panel body */
            .lesson-desktop-body { display: none !important; }

            /* Show mobile tab bar */
            .lesson-mobile-tabs { display: flex !important; }

            /* Show mobile tab content */
            .lesson-mobile-tab-content { display: flex !important; }

            /* Tighten top bar */
            .lesson-topbar-title { display: none; }
            .lesson-topbar-right-text { display: none; }
          }

          @media (min-width: 769px) and (max-width: 1100px) {
            .lesson-left-panel { width: 42% !important; min-width: 260px !important; }
            .lesson-output-panel { width: 24% !important; min-width: 180px !important; }
          }
        `}</style>
        ...
        <header className="flex items-center justify-between px-4 h-12 bg-[#16161e] border-b border-[#2e2e3e] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="secondary" className="lesson-topbar-title bg-[#2e2e3e] text-white border-0 font-semibold text-xs">
              {lesson.title}
            </Badge>
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" className="lesson-topbar-right-text text-[#aaa] hover:text-white hover:bg-[#2e2e3e] text-xs h-7">
              Get Unstuck
            </Button>
            <Button variant="ghost" size="sm" className="lesson-topbar-right-text text-[#aaa] hover:text-white hover:bg-[#2e2e3e] text-xs h-7">
              Tools
        ...
        {/* ── DESKTOP body (3-panel) ───────────────────────────────── */}
        <div className="lesson-desktop-body flex flex-1 overflow-hidden">

          {/* Left panel: lesson content */}
          <div className="lesson-left-panel w-[38%] min-w-[300px] bg-white flex flex-col overflow-hidden border-r border-[#2e2e3e]">
          ...
          {/* Monaco panel */}
          <div className="lesson-editor-panel flex-1 flex flex-col bg-[#1e1e2e] min-w-0 border-r border-[#2e2e3e]">
          ...
          {/* Output panel */}
          <div className="lesson-output-panel w-[28%] min-w-[220px] flex flex-col bg-[#0d0d14]">
            <div className="flex items-center justify-between px-4 h-10 bg-[#1a1a28] border-b border-[#2e2e3e] shrink-0">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-[10px] border-[#2e2e3e] text-[#555] font-mono py-0 h-5">
                  {langLabel}
                </Badge>
                {output !== null && (
                  <Button
            ...
            <ScrollArea className="flex-1">
              {output === null ? (
                ...
                <pre className={`p-4 text-[13px] leading-relaxed font-mono whitespace-pre-wrap break-words
        ...
        {/* ── MOBILE body (tabbed) ─────────────────────────────────── */}
        <div className="lesson-mobile-tab-content hidden flex-1 flex-col overflow-hidden">

          {/* Tab bar */}
          <div className="lesson-mobile-tabs hidden items-center bg-[#16161e] border-b border-[#2e2e3e] shrink-0">
            {([
              { id: 'learn',  label: 'Learn',  Icon: BookOpen  },
              { id: 'code',   label: 'Code',   Icon: Code2     },
              { id: 'output', label: 'Output', Icon: Terminal  },
            ] as { id: MobileTab; label: string; Icon: any }[]).map(({ id, label, Icon }) => (
              <button
                key={id}
                ...
                style={{
                  ...
                  padding: '10px 0',
                  fontSize: 13,
                  fontWeight: mobileTab === id ? 700 : 400,
                  color: mobileTab === id ? '#fff' : '#666',
                  borderBottom: mobileTab === id ? '2px solid #5b4fcf' : '2px solid transparent',
                  background: 'transparent',
                  border: 'none',
                  borderBottomStyle: 'solid',
                  borderBottomWidth: 2,
                  borderBottomColor: mobileTab === id ? '#5b4fcf' : 'transparent',
                  cursor: 'pointer',
          ...
          {mobileTab === 'learn' && (
            <div className="flex-1 bg-white overflow-hidden flex flex-col">
              <ScrollArea className="flex-1">
                <div
                  className="lesson-content px-5 py-5 text-sm leading-relaxed text-[#2a2a2a]"
                  dangerouslySetInnerHTML={{ __html: lesson.content }}
                />
              </ScrollArea>
            </div>
